# 👋 Hey there, I'm Rey Martín

I'm a **Full Stack Developer** focused on building scalable, production-ready web applications.  
I enjoy working with modern frontend architectures, backend APIs, automation, and DevOps workflows.

Currently working mainly with **Vue 3 (Pinia, Router, Vitest), Node.js, Taildwind, Apollo**.

---

## 🚀 What I Do

- Build full-stack web applications used in real production environments  
- Design clean frontend architectures with reusable components  
- Develop REST & GraphQL APIs with authentication and role-based access  
- Automate workflows, reports, and internal processes  
- Deploy and maintain applications on Linux VPS with CI/CD  

---
### Main Tech

---

## 📊 GitHub Activity

### 📈 Stats
![Rey's GitHub stats](https://github-readme-stats.vercel.app/api?username=Bluesfire1467&show_icons=true&theme=transparent)

### 🧠 Most Used Languages
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Bluesfire1467&layout=compact&theme=transparent)

---

## 🗓️ Coding Habits

- 💻 Regular commits during weekdays  
- 🚀 Strong activity on backend & frontend projects  
- 🔧 Frequent refactors and incremental improvements  
- 📦 Focus on long-term maintainable codebases
- 📊 Learning abouts advanced automatic testing

---

## 📫 Let's Connect

- 💼 LinkedIn: [linkedin.com/in/reyqg](https://linkedin.com/in/reyqg)  
- 📧 Email: rmquintero60@gmail.com  

---

> _I enjoy building systems that are simple to use, reliable to run, and easy to maintain._
